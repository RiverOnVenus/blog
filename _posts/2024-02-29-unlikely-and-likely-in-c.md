...
这个 `unlikely` 让我很疑惑，`if` 里面还能这样？然后在网上查了资料，也在 Linux 内核代码里发现了大量的 `unlikely`的使用。
...
既然知道了作用，可不可以通过程序验证是否有优化呢？比如通过时间复杂度高的查找算法在数据级大的样本里查找，然后比较花费的时间，不知道这样是否可以验证，试一试，这是一个查找素数的代码：
...
但是测出来大多数时候也是 `with_attributes` 更耗时。也许需要更专业的工具来进行测试。
